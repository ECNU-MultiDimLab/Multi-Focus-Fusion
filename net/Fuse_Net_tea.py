...
class text_preprocess(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv = nn.Conv1d(in_channel, out_channel, 1, 1, 0)     #使用一维卷积层对文本数据特征进行降维
...
class restormer_cablock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv1 = nn.Conv2d(input_channel, restormerdim, kernel_size=3, stride=1, padding=1, bias=bias)
        ...
        self.restormer1 = Restormer(restormerdim, restormerhead, ffn_expansion_factor, bias, LayerNorm_type)
        self.cross_attention = CrossAttention(embed_dim=hidden_dim, num_heads=8)
        self.imagef2textf1 = imagefeature2textfeature(restormerdim, image2text_dim, hidden_dim)

    def forward(self, image, text):
        b, _, H, W = image.shape
        image = self.restormer1(self.prelu1(self.conv1(image)))
        imagetotext = self.imagef2textf1(image)
        ca = self.cross_attention(imagetotext, text, text)     #(B,L,512)
        image_sideout = image
        ca = torch.nn.functional.adaptive_avg_pool1d(ca.permute(0, 2, 1), 1).permute(0, 2, 1)
        ca = F.normalize(ca, p=1, dim=2)
        ca = (imagetotext * ca).view(image.shape[0], self.image2text_dim, 288, 384)
        image_sideout = F.interpolate(image_sideout, [H, W], mode='nearest')
        ca = F.interpolate(ca, [H, W], mode='nearest')
        ca = self.prelu3(
            self.conv3(torch.cat(
                (F.interpolate(image, [H, W], mode='nearest'), self.prelu2(self.conv2(ca)) + image_sideout), 1)))
        return ca, imagetotext

class Text_Attention(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(Text_Attention, self).__init__()
    ...
    def forward(self, x):
        ...
        K = self.key_linear(x)  # (batch_size, seq_len, hidden_dim)
        V = self.value_linear(x)  # (batch_size, seq_len, hidden_dim)
        attention_scores = torch.matmul(Q, K.transpose(-2, -1))  # (batch_size, seq_len, seq_len)
        attention_weights = F.softmax(attention_scores, dim=-1)  # (batch_size, seq_len, seq_len)
        weighted_values = torch.matmul(attention_weights, V)  # (batch_size, seq_len, hidden_dim)
        output = self.layer_norm(weighted_values)  # (batch_size, seq_len, hidden_dim)
        return output
...
class Net(nn.Module):
    def __init__(
        ...
        self.Text_Attention = Text_Attention(hidden_dim, hidden_dim)
        self.restormerca_1 = restormer_cablock(input_channel=mid_channel, hidden_dim=hidden_dim, image2text_dim=image2text_dim)
        self.restormerca_2 = restormer_cablock(input_channel=mid_channel, hidden_dim=hidden_dim, image2text_dim=image2text_dim)
        self.restormerca_3 = restormer_cablock(input_channel=mid_channel, hidden_dim=hidden_dim, image2text_dim=image2text_dim)
        ...
        self.CrossAttentionFusion = CrossAttentionFusion(input_channel=mid_channel, image2text_dim=image2text_dim)

    def forward(self, imageA, imageB, textA, textB):
        Basic_Text_Feature_A = self.text_process(textA)
        Basic_Text_Feature_B = self.text_process(textB)
        Detail_Text_Feature_A = self.Text_Attention(Basic_Text_Feature_A)
        Detail_Text_Feature_B = self.Text_Attention(Basic_Text_Feature_B)
        ...
        Basic_Feature_A, _ = self.restormerca_1(Basic_Image_Feature_A, Basic_Text_Feature_A)
        x5 = Basic_Feature_A
        Basic_Feature_A, _ = self.restormerca_2(Basic_Feature_A, Basic_Text_Feature_A)
        x6 = Basic_Feature_A
        Basic_Feature_A, _ = self.restormerca_3(Basic_Feature_A, Basic_Text_Feature_A)
        x7 = Basic_Feature_A
        Detail_Feature_A, _ = self.restormerca_1(Detail_Image_Feature_A, Detail_Text_Feature_A)
        x8 = Detail_Feature_A
        Detail_Feature_A, _ = self.restormerca_2(Detail_Feature_A, Detail_Text_Feature_A)
        x9 = Detail_Feature_A
        Detail_Feature_A, _ = self.restormerca_3(Detail_Feature_A, Detail_Text_Feature_A)
        x10 = Detail_Feature_A
        Basic_Feature_B, _ = self.restormerca_1(Basic_Image_Feature_B, Basic_Text_Feature_B)
        x11 = Basic_Feature_B
        Basic_Feature_B, _ = self.restormerca_2(Basic_Feature_B, Basic_Text_Feature_B)
        x12 = Basic_Feature_B
        Basic_Feature_B, _ = self.restormerca_3(Basic_Feature_B, Basic_Text_Feature_B)
        x13 = Basic_Feature_B
        Detail_Feature_B, _ = self.restormerca_1(Detail_Image_Feature_B, Detail_Text_Feature_B)
        x14 = Detail_Feature_B
        Detail_Feature_B, _ = self.restormerca_2(Detail_Feature_B, Detail_Text_Feature_B)
        x15 = Detail_Feature_B
        Detail_Feature_B, _ = self.restormerca_3(Detail_Feature_B, Detail_Text_Feature_B)
        ...
        Cross_Feature_A = self.CrossAttentionFusion(Basic_Feature, Detail_Feature)
        x17 = Cross_Feature_A
        Cross_Feature_B = self.CrossAttentionFusion(Detail_Feature, Basic_Feature)
        x18 = Cross_Feature_B


        intermediate_outputs = (x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18)
        fusionfeature = torch.cat((Cross_Feature_A, Cross_Feature_B), dim=1)
