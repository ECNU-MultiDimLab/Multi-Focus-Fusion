...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, imageA, imageB):
        ...
        ca = (imagetotextA * ca).view(imageA.shape[0], self.image2text_dim, 128, 128)
...
class Net(nn.Module):
    def __init__(
        ...
        self.Base3 = TransformerBlock(dim=mid_channel, num_heads=decoder_num_heads, ffn_expansion_factor=2, bias=bias,
                                      LayerNorm_type=LayerNorm_type)
        self.Base4 = TransformerBlock(dim=mid_channel, num_heads=decoder_num_heads, ffn_expansion_factor=2, bias=bias,
                                      LayerNorm_type=LayerNorm_type)
        self.adjust_channel1 = nn.Sequential(
            nn.Conv2d(mid_channel * 2, mid_channel, kernel_size=1),
            nn.BatchNorm2d(mid_channel),
            nn.LeakyReLU(negative_slope=0.01)
        )
        self.adjust_channel2 = nn.Sequential(
            nn.Conv2d(mid_channel * 2, mid_channel, kernel_size=1),
            nn.BatchNorm2d(mid_channel),
            nn.LeakyReLU(negative_slope=0.01)
        )
    ...
    def forward(self, imageA, imageB):
        ...
        '''Fuse_Feature_A = torch.cat((Basic_Feature, Detail_Feature), dim=1)
        Fuse_Feature_A = self.adjust_channel1(Fuse_Feature_A)
        Fuse_Feature_A = self.Base4(Fuse_Feature_A)
        x17 = Fuse_Feature_A
        Fuse_Feature_B = torch.cat((Detail_Feature, Basic_Feature), dim=1)
        Fuse_Feature_B = self.adjust_channel2(Fuse_Feature_B)
        Fuse_Feature_B = self.Detail4(Fuse_Feature_B)
        x18 = Fuse_Feature_B'''
        ...
        '''for i, tensor in enumerate(intermediate_outputs, start=1):
